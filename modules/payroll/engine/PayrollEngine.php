...

{
    ...
    const RESIGNATION_PIT_RATE = 0.10; // Thuế TNCN 10% khi có ngày nghỉ việc
...

    {
        ...
        $responsibilityAllow = (int)($salary['responsibility'] ?? 0);
        $seniorityAllow      = (int)($salary['seniority']      ?? 0);
        ...
        foreach ($salary['all_components'] as $comp) {
            ...
            if (in_array($code, [
                ...
                'responsibility', 'seniority', // ✅ loại ra để không tính trùng vào OT base
        ...
        $resignationPitAmount = $hasResignation
            ? (int)round($grossForTax * self::RESIGNATION_PIT_RATE)
            : 0;
        $pitAmount = $hasResignation
            ? $resignationPitAmount
            : $this->calcPIT($taxableIncome);
        ...
        if ($hasResignation)
            $remarkParts[] = "Nghỉ việc: thuế TNCN = 10% × lương gross (-".number_format($resignationPitAmount)." đ)";
